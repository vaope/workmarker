...
def _main_impl() -> None:
    ...
    handlers = {
        ...
        "compendium_validate": handle_compendium_validate,
        "compendium_generate": handle_compendium_generate,
        "compendium_preview": handle_compendium_preview,
        "compendium_list": handle_compendium_list,
    }
...
def handle_commit(request: dict) -> dict:
    ...
    # 6. Enqueue classification for Phase B synthesis
    from workeventagent.classify_outbox import enqueue_classification
    classify_dir = project_dir / ".workeventagent" / "classify_outbox"
    enqueue_classification(classify_dir, project_path.stem, event_id)
...
# ── Phase B synthesis handlers ──────────────────────────────


def handle_list_proposals(request: dict) -> dict:
    """List synthesis proposals for a project, filtered by state."""
    project_path = Path(request["project_path"])
...
def handle_synthesis_apply(request: dict) -> dict:
    """Apply a validated synthesis proposal atomically."""
    project_path = Path(request["project_path"])
    workspace = project_path.parent
    store = workspace / ".workeventagent" / "proposals"
    proposal_id = request["proposal_id"]
...
def handle_panorama_synthesis(request: dict) -> dict:
    """Generate a current-panorama synthesis proposal for recent events."""
    project_path = Path(request["project_path"])
    workspace = project_path.parent
    store = workspace / ".workeventagent" / "proposals"
    opencode_bin = request.get("opencode_bin", "opencode")
    ...
    event_ids = tuple(e["event_id"] for e in events[:10])
...
def handle_reviewed_synthesis(request: dict) -> dict:
    """Generate a reviewed section synthesis proposal."""
    project_path = Path(request["project_path"])
    workspace = project_path.parent
    store = workspace / ".workeventagent" / "proposals"
    section_id = request["section_id"]
...
def handle_trigger_classification(request: dict) -> dict:
    """Process pending classification jobs. Generates synthesis proposals for high-impact events."""
    project_path = Path(request["project_path"])
    workspace = project_path.parent
...
# ── Phase C compendium handlers ────────────────────────────────


def handle_compendium_validate(request: dict) -> dict:
    """Validate module conclusions before generating compendium."""
    project_path = Path(request["project_path"])

    from workeventagent.compendium import validate_module_conclusions
    return validate_module_conclusions(project_path)


def handle_compendium_generate(request: dict) -> dict:
    """Generate a project compendium snapshot."""
    project_path = Path(request["project_path"])
    workspace = project_path.parent
    opencode_bin = request.get("opencode_bin", "opencode")
    model = request.get("model", "")
    ...
    from workeventagent.compendium import generate_compendium
    return generate_compendium(
        project_path, workspace,
        opencode_bin=opencode_bin, model=model, use_ai=use_ai,
    )


def handle_compendium_preview(request: dict) -> dict:
    """Read a generated compendium for preview."""
    compendium_path = Path(request["compendium_path"])

    try:
        content = compendium_path.read_text(encoding="utf-8")
        return {"ok": True, "content": content, "path": str(compendium_path)}
    except (OSError, UnicodeDecodeError) as exc:
        return {"ok": False, "kind": "read_error", "error": str(exc)}


def handle_compendium_list(request: dict) -> dict:
    """List existing compendium snapshots for a project."""
    project_path = Path(request["project_path"])
    workspace = project_path.parent

    from workeventagent.compendium import list_compendiums
    snapshots = list_compendiums(workspace, project_path.stem)
