...
- 自动验证：`356 passed, 26 subtests passed`；11 个 client JavaScript 文件通过 `node --check`；真实 opencode 1.18.1 synthesizer bounded-JSON smoke 通过。模块标题在 adapter、builder 与 worker 持久化前统一拒绝 Unicode 行分隔符和不可打印字符。
